...
every 1.hours do
  ...
  users.each do |user|
    ...
    # 引数渡す
    return_value = command "python #{file}/app/controllers/concerns/hackason/exe_per_hour.py -i #{user.nickname} -hv #{hash.tweet_hash} -s #{hash.start_time} -e #{hash.end_time} -c #{hash.count}"
